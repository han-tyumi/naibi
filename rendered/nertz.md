...
**Jokers** — Two jokers go into each pack and stand for any card you like. Playing one to a foundation carries a protocol: the rank and suit it stands for have to be called out while it is still sitting on its pile, which gives everybody else a fair chance to get the real next card down on top of it first. On the work piles no announcement is needed and a joker can go anywhere — but once a real card is laid on it, its rank and colour are fixed until that card comes off again. Jokers score like any other card.
