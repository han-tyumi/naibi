...
**Paying for the margin** — Many players add a payment for overtricks, which the basic game ignores completely. The usual arrangement reprices a solo at four, five or six units and scales everything else to match, after which each trick above or below the contract in a prop and cop or a solo moves one more unit. Abundance is deliberately lopsided: an overtrick earns two, while an undertrick costs only one. The three all-or-nothing contracts — misère, misère ouverte and abundance declared — have no margin to pay for and are left alone.
...
**Reordering the ladder** — The order of the contracts is not settled everywhere. One account reports experienced players lifting misère above abundance in trumps and making misère ouverte the highest bid of all, above abundance declared, with the payments adjusted to suit — an admission that a misère is harder than its position suggests.
