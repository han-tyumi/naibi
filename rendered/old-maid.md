...
**Scabby Queen** — A British version, recorded in Scotland and under several other names across England. The queen of clubs is the card taken out, which leaves the two red queens to pair with each other and strands the queen of spades — the scabby queen — as the odd card. The pack is then brought down edgewise on the loser's knuckles, as many times as a card drawn from the reshuffled deck says: face value for a number card, ten for a jack or a king, twenty-one for a queen. A red card means the raps are soft and a black one means they are hard.

**Black Peter** — The continental European form. Every country has its own translation of Peter or Black Peter for it — Schwarzer Peter in Germany. Played with ordinary cards it uses a 32-card pack with one black jack removed, so the other black jack is the card left over. More often the pack is a purpose-made one of 31 or 37 cards whose odd card is a chimney sweep, or in other packs a black cat. The loser traditionally gets a black mark drawn on the face with soot or a burnt cork.
